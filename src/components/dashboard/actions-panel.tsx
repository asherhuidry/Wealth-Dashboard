    ...
    <section className="overflow-hidden rounded-[28px] border border-white/[0.07] bg-[#0d1527]/80 text-white shadow-[0_18px_42px_rgba(0,0,0,0.32)] backdrop-blur-xl">
      <div className="flex items-center justify-between border-b border-white/[0.06] px-6 py-5">
        <div>
          ...
          <p className="mt-0.5 text-[12px] text-white/42">Suggested moves with the highest expected leverage</p>
        </div>
      ...
      <div className="space-y-2.5 p-4">
        {actions.map((action, index) => {
          ...
          return (
            <article
              key={action.id}
              className="flex items-start gap-3.5 rounded-2xl border border-white/[0.07] bg-white/[0.025] px-4 py-3.5"
            >
              <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-xl bg-white/[0.06] text-[11px] font-bold text-white/40">
                {index + 1}
              </span>
              ...
              <svg
                ...
                stroke="currentColor"
                strokeWidth="1.5"
                className="mt-0.5 h-4 w-4 flex-shrink-0 text-white/24"
              >
